...
function math.frandom(low, high)
    if low and high then
        assert(type(low) == "number" and type(high) == "number", ("Number, number expected, got %s, %s."):format(type(low), type(high)))
        return math.random(low, high - 1) + math.random() --returns a value in low < value < high
    elseif low then
        assert(type(low) == "number", ("Number expected, got %s."):format(type(low)))
