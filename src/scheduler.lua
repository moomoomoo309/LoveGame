...
--- @tparam function conditionFct The function supplying the condition that will run fct.
--- @tparam function|nil fct (Optional) The function to run every time conditionFct returns a truthy value.
--- @tparam function|nil risingFct (Optional) The function to run every time conditionFct goes from false to true.
--- @tparam function|nil fallingFct (Optional) The function to run every time conditionFct goes from true to false.
--- @tparam function|nil cancelFct (Optional) A function to run if this one is cancelled.
--- @tparam string|nil group (Optional) The group the function should be in. Defaults to "default".
--- @treturn function a function which cancels this function.
function scheduler.everyCondition(conditionFct, fct, risingFct, fallingFct, cancelFct, group)
    assert(type(fct) == "function" or type(risingFct) == "function" or type(fallingFct) == "function", ("Function(s) expected, got %s, %s, %s."):format(type(fct), type(risingFct), type(fallingFct)))
    assert(type(conditionFct) == "function", ("Function expected, got %s."):format(type(conditionFct)))
    assert(type(cancelFct) == "function" or not cancelFct, ("Function or nil expected, got %s."):format(type(cancelFct)))
    group = group or "default"
    scheduler.functions[group] = scheduler.functions[group] or {}
    ...
    scheduler.functions[group][index] = function()
        if conditionFct() then
            timesRun = timesRun + 1
            if not lastRun and risingFct then
                risingFct(timesRun)
            elseif fct then
                fct(timesRun)
            end
            ...
            if lastRun and fallingFct then
                fallingFct(timesRun)
            end
            lastRun = false
        end
